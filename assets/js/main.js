...
*/
  ...
  function speak(isChatBot) {
      ...
      if (playBackTxt.value !== '') {
          ...
          utterThis.onend = function(event) {
              console.log('SpeechSynthesisUtterance.onend',counter);
              if(isChatBot){
  ...
  startChatBtn.onclick = function() {
    ...
    userGreeting = ' Welcome Geoff. How can we help you today?'
  ...
  var addChatToChatArea = function(isChatBot, text){
    ...
    if(isChatBot){
      ...
      speak(isChatBot);
      html = '<div><strong>'+chatBotName+'</strong> says : <I>'+text+'</I></div>'
    }else {
      console.log("-->1")
      if(text != null && text!==''){
        console.log("-->2")
        html = '<div><strong>'+userChatName+'</strong> says : <I>'+text+'</I></div>'
      }else {
        console.log("-->3")
  ...
  var findResponse= function(speechResult){
    ...
    var responseMapper = [
      ...
      {
        keywords: 'pay my bill#pay', //I want to pay my bil
        response:'Would you like an update on your bill?'  
      },
      {
        keywords: 'yes',
        response:'Your last bill was £19.99, Would you like me to email, text your statement'  
      },
      {
        keywords: 'email',
        response:'Sure we will send the statement to the registered email address : GeoffUnknown@unknown.com. Thanks for using our voice service. Have a nice day'  
      },
      {
        keywords: 'text#sms',
        response:'Sure we will send the statement to the registered mobile phone. Thanks for using our voice service. Have a nice day'  
      }
